...
public class Scheduler {
    ...
    String latestJavaTmp;

    Map<Mutators, Integer> hitTimes = new HashMap<>();

    public static List<String> selMutNames = new ArrayList<>();
    String prevMut = SchedulerV2.HEAD_MUT;

    public static boolean hasScore = false;
    public static List<Long> pidList;
    public static List<String> logBuff = new ArrayList<>();

    static {
        pidList = new ArrayList<>();
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            Scheduler.log.debug("MopFuzzer is shutting down. Cleaning up resources if necessary.");
            for (Long pid: Scheduler.pidList) {
                try {
                    ProcessBuilder pb = new ProcessBuilder("kill", "-9", String.valueOf(pid));
                    pb.start();
                    Scheduler.log.debug("MopFuzzer killed process with pid: " + pid);
                } catch (Exception e) {
                    Scheduler.log.debug("MopFuzzer failed to kill process with pid: " + pid + " due to exception: " + e.toString());
                }
            }
        }));            
    };

    public static void logSth(String s) {
        if (logBuff != null) {
            logBuff.add(s);
        } else {
            log.debug(s);
    ...
    public static String rcovRunCmd(String cmd) {
        if (cmd == null || cmd.trim().isEmpty()) {
            return "";
        }
        try {
            ProcessBuilder pb = new ProcessBuilder("/bin/sh", "-c", "timeout " + Configuration.LinuxTimeout + "s " + cmd);
            ...
            return "Command Execution Internal Exception: " + t.getMessage();
        }
    }
    ...
    private Patch generateAndValidatePatches(CodeNode cn, Mutators mutator) throws IOException, InterruptedException {
        ...
        if (patch == null) {
            log.debug("mutator " + mutator.name() + " fails to generate patch for line " + Configuration.lineNumber
                    + " and iter " + this.curIter);
            return null;
        }
    ...
    private static void configureLogging(String logFilePath) {
        ...
        // loggerContext.reset();
        if (logBuff != null) {
            log.debug("Start log buffer flush:");
            for (String logEntry : logBuff) {
                log.debug(logEntry);
            }
            log.debug("End log buffer flush.");
            logBuff = null;
        }
    ...
    public void run() {
        ...
        prepare(config);
        log.debug("Target case is " + config.targetCase + ". V2Scheduler enabled: " + Configuration.enableScheduler + ". Rcov run any way enabled: " + Configuration.enableRcovRunAnyWay);
        if (Configuration.enableScheduler) {
            log.debug("V2Scheduler selects seed " + config.javaFilePath + " for mutation.");
        }
        ...
        // SchedulerV2.lockTestingSeed(config.fullSrcName);
        try {
            int infiniteLoop = 0;
            ...
            while (curIter <= Configuration.maxIter) {
                ...
                Mutators select = null;
                if (!Configuration.enableScheduler) {
                    select = getRandomElement(satisfiableMutators);
                } else {
                    List<String> mutNames = new ArrayList<>();
                    List<Mutators> mutList = new ArrayList<>(satisfiableMutators);
                    for (Mutators mut : mutList) {
                        mutNames.add(mut.name());
                    }
                    Integer selIdx = SchedulerV2.selectMut(config.javaFilePath, mutNames, prevMut);
                    String selMut = mutList.get(selIdx).name();
                    select = mutList.get(selIdx);
                    selMutNames.add(selMut);
                    log.debug("V2scheduler selected mutators: " + selMut + " for seed " + config.javaFilePath);
                    log.debug("V2scheduler selected mutator: " + select.name() + " for seed " + config.javaFilePath);
                }
                ...
                } else if (!Configuration.enableScheduler) {
                ...
                hit = hitTimes.get(select);
                hitTimes.put(select, Math.min(hit + 1, select.getMaxHitTimes()));
    ...
    public void run_rcov(String tmpDir, JDK jdk, boolean runOnly) {
        if (JDK.getRcovJavac() == null) {
            log.debug("V2Scheduler skips running rcov for test case because rcov is not available.");
            return;
        } else if (curIter + 1 > Configuration.maxIter) {
            log.debug("V2Scheduler skips running rcov for test case because max iterations are reached.");
            return;
        } else {
            try {
                String rcovCompileCmd = JDK.getRcovJavac() + " -cp \""
                    + (!config.isRegression ? config.javaFuzzerRoot : jdk.getTestLibRoot())
                    + "\"  -d " + tmpDir + " " + this.latestJavaTmp;
                log.debug("rcov compiling with command: \n" + rcovCompileCmd);
                Process p1 = runCmd(rcovCompileCmd);
                ShellUtils.getShellOut(p1, 1, tmpDir + "rcov_javac_log.txt");
                String cp = iter0Path == null ? (Configuration.isRegression ? jdk.getTestLibRoot() : Configuration.javaFuzzerRoot)
                : iter0Path;
                String rcovRunCmd = JDK.getRcovJava() + " -cp \"" + 
                    tmpDir + jdk.classPathSplitter + cp + "\" " + trueTestCase;
                log.debug("rcov running with command: \n" + rcovRunCmd);
                File rcovDataPath = new File(Paths.get("").toAbsolutePath().toString() + "/" + tmpDir + "route_data.log");
                String ret = rcovRunCmd(rcovRunCmd);
                FS.writeString(rcovDataPath.getAbsolutePath(), ret);
                log.debug("logs will be writen to: " + rcovDataPath.getAbsolutePath());
                try {
                    log.debug("Waiting for rcov to generate log file...");
                    Thread.sleep(5000);
                } catch (InterruptedException e) {
                    e.printStackTrace();
                }
                int exitCode = 0;
                if (ret.startsWith("TIMEOUT")) {
                    exitCode = 124;
                } else {
                    exitCode = 1;
                }
                if (rcovDataPath.exists()) {
                    if (!runOnly) {
                        log.debug("V2Scheduler is scoring test case with rcov coverage report.");
                        SchedulerV2.scoreTestCase(tmpDir, rcovDataPath.getAbsolutePath(),
                            config.javaFilePath, selMutNames, this.latestJavaTmp, exitCode);
                        hasScore = true;
                        log.debug("V2Scheduler finished scoring test case with rcov coverage report.");
                    } else {
                        log.debug("V2Scheduler skips scoring test case with rcov because runOnly is true.");
                    }
                } else {
                    log.error("V2Scheduler fails to score test case with rcov because the log file is not generated.");
                }
                log.debug("Finished running rcov for test case.");
            } catch (IOException e) {
            }
        }
    }
        ...
        if ((this.curIter % intervals == 0 && !isFinal) || (isFinal && this.curIter % intervals != 0)) {
            try {
                for (int i = 0; i < config.jdks.length; i++) {
                    ...
                    if (Configuration.enableScheduler) {
                        log.debug("Running rcov for test case to generate coverage report for V2Scheduler.");
                        run_rcov(tmpDir, jdk, false);
                        log.debug("Finished running rcov for test case to generate coverage report for V2Scheduler.");
                    } else if (Configuration.enableRcovRunAnyWay) {
                        log.debug("Running rcov for test case to generate coverage report forcely.");
                        run_rcov(tmpDir, jdk, false);
                        log.debug("Finished running rcov for test case to generate coverage report forcely.");
                    } else {
                        log.debug("V2Scheduler skips running rcov for test case because scheduler is not enabled.");
                    }
                }
