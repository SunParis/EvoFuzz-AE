...
class JFuzz(JavaGenerator):
    ...
    def __init__(self, home: Path, out_dir: Path, seed: Optional[int] = None):
        ...
        script_check(self.jfuzz is None or self.jfuzz.exists(), f'Binary jfuzz does not exist in {JFuzz.NAME}\'s home: {self.home}')
...
class ExistingTests2(JavaGenerator):
    ...
    def __init__(self, test_dir: Path, out_dir: Path):
        super(ExistingTests2, self).__init__()
        self.test_dir = test_dir
        self.out_dir = out_dir
        self.file_list = list_all_files_abs(str(test_dir), '.java')
        self._curr_index = 0

    def __next__(self) -> Path:
        import random
        target = random.choice(self.file_list)
        self._curr_index += 1
        
        # Copy to out_dir and return the java file in out_dir
        copy_into_dir = str(self.out_dir / str(self._curr_index))
        ensure_dir(copy_into_dir)
        copy_into(str(Path(__file__).absolute().parent / 'java_fuzzer/lib/FuzzerUtils.java'), copy_into_dir)
        succ = copy_into(target, copy_into_dir)
        script_check(succ, f'Fail to copy directory to {copy_into_dir}')
        return Path(copy_into_dir) / Path(target).name
...
def parse_generator_conf(key_path: str, gen_conf: dict) -> dict:
    ...
    elif which_gen == ExistingTests2.NAME:
        pass
    else:
...
def create_generator_from_conf(gen_conf: dict, java_fuzzer_path: Optional[Path], java_fuzzer_default_conf: Optional[Path]):
    if gen_conf['name'] == JavaFuzzer.NAME:
        if java_fuzzer_path is None or java_fuzzer_default_conf is None:
            raise ValueError(f"Generator {JavaFuzzer.NAME} is selected but JAVA_FUZZER_PATH or JAVA_FUZZER_DEFAULT_CONF is not set")
    ...
    elif gen_conf['name'] == ExistingTests2.NAME:
        java_gen = ExistingTests2(gen_conf['exist_dir'], gen_conf['out_dir'])
    else:
...
def run_test(
    ...
    num_mutation: int, run_timeout: int,
    seed_bkp_dir: Optional[Path] = None,
    exist_seed_dir: Optional[Path] = None
) -> TestResult:
    compilation_timeout = 3600  # seconds
    mutation_timeout = 3600     # seconds
    
    def backup_ref_file(ref: Path, bkp_dir: Path):
        import os
        import shutil
        ...
        while os.path.exists(dst):
            dst = os.path.join(bkp_dir, f"{name}_{counter}{ext}")
            counter += 1            
        shutil.copy(ref, dst)
        return dst

    if exist_seed_dir is None and seed_bkp_dir is not None:
        backup_ref_file(ref_file, seed_bkp_dir)
...
class WriterStat:
    @abstractmethod
    def ref_count(self) -> int: pass

    @abstractmethod
    def inc_ref_count(self): pass

    @abstractmethod
    def mut_count(self) -> int: pass
    ...
    @abstractmethod
    def mutant_timeout_count(self) -> int: pass

    @abstractmethod
    def inc_mutant_timeout_count(self): pass

    @abstractmethod
    def timeout_count(self) -> int: pass
...
class AxExecutor(MprExecutor[Path, TestResult]):
    ...
    def __init__(self,
        ...
        self.seed_bkp_dir = seed_bkp_dir
        self.exist_seed_dir = Path(os.path.abspath(str(exist_seed_dir)))

    def __call__(self, ref_id: int, ref_file: Path) -> Optional[TestResult]:
        return run_test(ref_file, self.jvm, self.ax, self.num_mutation,
            self.run_timeout, self.seed_bkp_dir, self.exist_seed_dir)

    def should_early_exit(self, i: int, t: Path) -> bool:
...
def main(
    conf_path: Path,
    java_fuzzer_path: Optional[Path],
    java_fuzzer_default_conf: Optional[Path],
    seed_bkp_dir: Optional[Path] = None,
    exist_seed_dir: Optional[Path] = None
):
    ...
    java_gen = create_generator_from_conf(conf['generator'], java_fuzzer_path, java_fuzzer_default_conf)
    ...
    mprunner = MultiProcRunner(num_proc, java_gen,
                               AxExecutor(jvm, artemis, num_mutation, prog_timeout, seed_bkp_dir, exist_seed_dir),
                               AxHandler(writer))
    ...
    if mprunner.is_stopped_normally():
        print(f'Found {stat.diff_count()}/{stat.mut_count()} differences in {format_time(int(elapsed))}')
...
def main_process(conf_path: str,
    java_fuzzer_path: Optional[Path] = None,
    java_fuzzer_default_conf: Optional[Path] = None,
    seed_bkp_dir: Optional[Path] = None,
    exist_seed_dir: Optional[Path] = None
):
    main(Path(conf_path), java_fuzzer_path,
        java_fuzzer_default_conf, seed_bkp_dir, exist_seed_dir)
