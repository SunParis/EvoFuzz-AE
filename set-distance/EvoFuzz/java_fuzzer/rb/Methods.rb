...
#               inner_callees.each{|inner_callee| @context.addMethodCaller(self,inner_callee) unless (inner_callee.nil?)} 
    ...
    def gen
        ...
        res += ln("long meth_res = 0;") if !@mainFlag # no declarations should be generated for main method
...
#main method:
        if @mainFlag            
            ...
            shift(-1)
            res += ln(" } catch (OutOfMemoryError ex) {")
            shift(1)
            res += ln("ex.printStackTrace(FuzzerUtils.err);")
            res += ln("System.exit(1);")
            shift(-1)
...
def rInvocation(type=nil)
    ...
    res = meth.name + '('
    meth.args.each {|arg|
        if (arg.type != rExp.type || arg.type.nil? || rExp.type.nil? || arg.type == 'char' || arg.type == 'byte' || arg.type == 'short')
          res += '(' + arg.type + ')(' + rExp + '), '
        else
          res += rExp + ', '
        end
...
#was:        @method = stmt.context.method.methClass.getMethod(stmt.context.method, type) if @method.nil?
    ...
    def gen
        ...
        @operands.each_with_index do |exp, i|
          if (exp.type != @method.args[i].type || @method.args[i].type.nil? || exp.type.nil? || @method.args[i].type == 'char' || @method.args[i].type == 'byte' || @method.args[i].type == 'short')
            res += '(' + @method.args[i].type + ')(' + exp.gen() + '), '
          else
            res += exp.gen() + ', '
          end
        end
...
class ReturnStmt < Statement
    ...
    def gen
        ...
        elsif @context.method.type == 'String'
            res += ln('return String.valueOf(' + checkSum + ');')
        else
...
class SmallMethStmt < Statement
    ...
    def gen
        ...
        res = prefix + ln('for (int smallinvoc=0; smallinvoc<'+ 
                loop_length.to_s+ '; smallinvoc++) {')
        shift(1)
        res += ln(@invocExpr.gen() + ';')
        shift(-1)
        res += ln('}')
        if $conf.outer_control
            shift(-1)
