...
class Var
    ...
    def initialize(cont, type="", flags=0, className=nil, name=nil)
        ...
        @init_value = 0
        if @type == 'Object' and !flag?(Nam::NOTNULL)
            @flags |= Nam::NOTNULL
        end
    end
...
class Context
    ...
    def addMethodCaller(caler, callee=nil, hash=nil)
        ...
        if caler==callee
          printMethodCallers()
          error("Fatal! trying to cycle a call chain: callee \"#{callee.name}\" is already a caller of caller \"#{caler.name}\", case 1, callee_nil = #{callee_nil}\n", false, true)
          exit(1)
        end
        return if caler==callee or callee.nil? or caler.nil?
        if caller?(callee, caler)
          printMethodCallers()
          error("Fatal! trying to cycle a call chain: callee \"#{callee.name}\" is already a caller of caller \"#{caler.name}\", case 2\n", false, true)
          exit(1)
        end
...
def error(message, internal=false, fatal=true)
    $stderr.puts "Fuzzer: " + (internal ? "Internal error" : "Error") + "! #{message}\n"
    exit(1) if fatal
end
