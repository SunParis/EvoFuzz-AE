...
class SwitchStmt < Statement
    ...
    def initialize(cont, par)
        ...
        if !packed
            ...
            # print("value2" + value.gen() + "\n")
        end
        if minValue > 0
            # print("value" + value + "\n")
            # print("value.resType" + value.resType + "\n")
            # print("minValue" + minValue.to_s + "\n")
            value = Expression.new(self, value.resType, 2, 'oper', {'op'=>Operator.get('+', 'infix','arith'), 'vals'=>[value,
                ExpIntLit.new(self, minValue.to_s, 3)]})
            # print("value3" + value.gen() + "\n")
        end
        @valueExpr = value
        numBreaks = 0
        @caseCount.times {|i|
    ...
    def gen
        # switch 只支持 int、byte、short、char、String 和枚举类型
        # 如果表达式类型大于 int（如 long, float, double），需要强制转换为 int
        expr = @valueExpr.gen()
        
        # 根据表达式的结果类型决定是否需要类型转换
        if typeGT?(@valueExpr.resType, "int")
            # long, float, double 等类型需要转换为 int
            val = "(int)(" + expr + ")"
        elsif @valueExpr.resType == "int"
            # int 类型直接使用
            val = expr
        else
            # byte, short, char 会自动提升为 int，不需要显式转换
            val = expr
        end
