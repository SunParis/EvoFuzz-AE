...
def defOperators
    $conf.operators.keys.each {|category|
        $conf.operators[category].values.each {|op|
            ...
            elsif category == 'string_oper'
                OP_LIST << Operator.new(op, category, TSET_STRING,     'infix',   TSET_STRING, TSET_STRING)
            elsif category == 'string_assn'
                OP_LIST << Operator.new(op, category, TSET_STRING,     'infix',   TSET_STRING_VAR, TSET_STRING)
            elsif op == '!'
...
class Expression
    ...
    attr_reader :kind, :flags, :operands, :resType, :type, :operator
    ...
    def initialize(stmt, type=nil, depth=0, kind=nil, elements=nil, flags=0, className=nil)
        ...
        # print("exp_kind" + $conf.exp_kind.to_s + "\n")
        # print("kind" + @kind.to_s + "\n")
        # print("getCallersHashDepth()" + $globalContext.getCallersHashDepth().to_s + "\n")
        # print("depth" + depth.to_s + "\n")
    ...
    def operResult(depth)
        ...
        @operator = Operator.get($conf.operators[opCat].getRand(), nil, opCat)
        # print("@operator.operand1TypeSet[0]" + @operator.operand1TypeSet[0].to_s + "\n")
        # print("opCat" + opCat + "\n")
        # print("operands[0].sign" + @operands[0].sign + "\n")
        # print("operands[0]" + @operands[0].gen() + "\n")
        if ['String_var', 'boolean_var', 'integral_var', 'arith_var', 'object_var', 'array_var'].member?(@operator.operand1TypeSet[0])
            @operands << Expression.new(@parentStmt, @type, depth+1, $conf.exp_kind.getRand(['scalar', 'array'],excl()), nil, Exp::DEST|notnullflag, @className)
        else
    ...
    def arrElem(depth)
        ...
        @operator.ndim.times {|i|
            ...
            if indKind == '0'
                ...
                op = Operator.get(indKind == '-1' ? '-' : '+', 'infix','arith')
    ...
    def genVal(expr, op=nil)
        ...
        return res ##############
        # Never reached because of return above
    ...
    def gen
        ...
        when 'scalar', 'arrname'
            if @className.nil? or @type!='Array'
                ...
                # @operands[1..-1].each{ |op| 
                #     if op.instance_of?(String)
                #         res+="["+op+"]"
                #     else
                #         index_expr = op.gen()
                #         # 如果索引表达式的结果类型大于 int，需要强制转换为 int
                #         if typeGT?(op.resType, 'int')
                #             res+="[(int)("+index_expr+")]"
        ...
        when 'array'
        #     indices = @operands.collect{|exp| 
        #         index_expr = exp.gen()
        #         # 如果索引类型大于 int，需要强制转换
        #         typeGT?(exp.resType, 'int') ? "(int)(#{index_expr})" : index_expr
        #     }
        #     # print("indices" + indices.join("][") + "\n")
        #     res = @operator.name + "[" + indices.join("][") + "]"
        # # when 'oper', 'assign'
        ...
        when 'oper', 'assign'
            ...
            if !@operands[0].instance_of?(String) and !@operands[1].instance_of?(String)
                if ['assign'].member?(@kind) && @operands[0].resType != @operands[1].resType
                    # If types are different in assignment, cast the right operand to the left operand's type
                    res += '(' + @operands[0].resType + ')'
                elsif ( ['integral_assn', 'arith_assn'].member?(@operator.category) and
...
class AssignmentStmt < Statement
    ...
    def initialize(cont, par, expr=nil)
        ...
        # Handle String += String to prevent OOM
        if @assnExpr && @assnExpr.resType == 'String' && @assnExpr.operator.sign == '+='
            # Get the destination variable
            dest_var = @assnExpr.operands[0]
            if dest_var && dest_var.kind == 'scalar'# && wrand([true, true, true, true, false])  # 80% chance to add reset
                # Create a short string literal assignment before the += operation
                short_strings = ['"hhhh"', '""', '""', '""']
                short_str = wrand(short_strings)
                reset_expr = Expression.new(self, 'String', 0, 'assign', {
                    'op' => Operator.get('=', nil, 'string_assn'),
                    'vals' => [dest_var.dup, Expression.new(self, 'String', 1, 'literal', {'op'=>nil, 'vals'=>[short_str]}, 0)]
                })
                @resetExpr = reset_expr
            end
        end
    end
    ...
    def gen
        res = ""
        if @resetExpr
            res += ln("// Resetting String variable to a short value to prevent OOM")
            res += ln(@resetExpr.gen() + ";")
        end
        res + ln(@assnExpr.gen() + ";")
    end
end
