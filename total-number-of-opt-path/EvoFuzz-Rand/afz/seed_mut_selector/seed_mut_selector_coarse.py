...
class SeedMutSelectorCoarse(Selector):
    ...
    def select_mutator(self, seed_path: str) -> List[str]:
        ...
        for mut_data in mut_priority_data:
            if mut_data[MUT_TYPE_LABEL] == 'structure':
                all_muts.append(mut_data[MUT_PATH_LABEL])
            else:
                all_muts.append(mut_data[MUT_PATH_LABEL])
        ...
        for _ in range(struct_choose_num + stmt_choose_num):
            prev = self._select_mutator_with_ctx(seed_path, prev, all_muts)
            ret.append(prev)
        ...
        for mut in mut_list:
            if mut not in curr_list:
                ...
                if seed_mut_data.get(mut) is not None:
                    sc1 = HistoryScoreData(seed_mut_data[mut])
                
                for mut_data in priority_data:
                    if mut_data[MUT_PATH_LABEL] == mut:
                        sc2 = HistoryScoreData(mut_data[SCORE_LABEL])
                        break
                
                if sc1 and sc2:
                    colect_data.append([sc1, sc2])
        
        res = _select_top_k_with_ratio(colect_data, [8, 4])
        if len(mut_list) == 0:
