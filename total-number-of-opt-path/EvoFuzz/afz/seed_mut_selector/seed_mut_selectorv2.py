...
# Hyperparameters
ALPHA = 0.75 # The bigger ALPHA is, the faster the score will decay, and the more recent scores will be emphasized
TRANS_DECAY = 0.5 # The bigger TRANS_DECAY is, the faster the score will decay
C_EXPLORE = 2.0 # The bigger C_EXPLORE is, the more exploration will be emphasized during selection.
...
class SeedMutSelectorV2(Selector):
        ...
        for mut in mut_list:
            ...
            for mut_data in priority_data:
                if mut_data[MUT_PATH_LABEL] == mut:
                    sc1 = HistoryScoreData(mut_data[SCORE_LABEL])
                    break
            for mut_data in mut_mut_data:
                if mut_data[MUT_PATH_LABEL] == ctx:
                    sc2 = HistoryScoreData(mut_data[mut])
                    break
            if seed_mut_data.get(mut) is not None:
                sc3 = HistoryScoreData(seed_mut_data[mut])
    ...
    def _score_to_reward(self, score: Score) -> Tuple[float, float]:
        ...
        if score.new_path_num[0] > 0:
            nnn_path_num: float = xy_map(score.new_path_num[0])
            reward_mut += nnn_path_num * self.mut_prio_adjust.new_code_paths
            reward_seed += nnn_path_num * self.seed_prio_adjust.new_code_paths
