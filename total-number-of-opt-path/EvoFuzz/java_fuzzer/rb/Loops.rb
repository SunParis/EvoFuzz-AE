...
class ForLoopStmt < Statement
    ...
    def initialize(cont, par)
        ...
        varType = $conf.p_ind_var_type.getRand()
        @inductionVar = Var.new(@context, varType)
        @lockVar = Var.new(@context, 'Object')
        @inductionVar.inductionVarFlag = true
        # 将锁变量注册到上下文中，这样它会在 genDeclarations() 中生成
        # @context.registerVar(@lockVar, true, "lockVar")
    ...
    def gen
        ...
        inc  = ((@step == -1) ? "--" : " -= " + (-@step).to_s) if @step < 0
        # res1 = "Integer " + ivarLock + " = Integer.valueOf(" + ivar + ");\n"
        # res1 = "synchronized (" + ivarLock + ") {\n"
        res1 = "for (" + ivar + " = (" + @inductionVar.type + ") (" + init + "); "
        res1 += @condition.gen()
        res1 += "; "
...
class SyncStmt < Statement
    ...
    def initialize(cont, par)
        ...
        @maxVal = ForLoopStmt.indVarMaxValue(@parent, nil, self)
        @initVal = @inductionVar.init_value
        @initlockVar = @lockVar.init_value
        @nestedStmts["body"] = genStmtSeq($conf.max_loop_stmts, false) # at least 1 stmt
    end
